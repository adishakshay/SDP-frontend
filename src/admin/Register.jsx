...
const Register = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await axios.get('http://localhost:8082/event/getAll');
  ...
  const handleDelete = async (eventId) => {
    try {
        const response = await axios.delete(`http://localhost:8082/adminuser/register/${eventId}`);
  ...
  const handleSaveEdit = async () => {
    try {
      await axios.put(`http://localhost:8082/adminuser/register/${editEvent.eventId}`, {
        ...editEvent,
  ...
  const handleAddNewEvent = async () => {
    try {
      const response = await axios.post('http://localhost:8082/event/add', newEvent);
